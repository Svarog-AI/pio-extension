...
2. **Completion signal:** `COMPLETION_SUMMARY.md` replaces the empty `COMPLETED` marker at the goal level. Written by evolve-plan with YAML frontmatter (`status: "complete"`) and a markdown body explaining why the goal is complete. Enforced via a `OneOfGroup` contract entry with group-level `requiredWhen` (active when `stepNumber > totalSteps`) — mutually exclusive with `REVISE_PLAN_NEEDED.md` (workspace root), enabling evolve-plan to request a plan revision instead of completing.
