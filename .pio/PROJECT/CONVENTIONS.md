...
- **Comments are lean and load-bearing only:** Brief statements of non-obvious intent or binding forward handoffs (e.g., mandatory `// transitional:` markers for a future step). No design narratives, decision history, or spec/review context embedded in code or test comments — that content belongs in the `.pio/` workspace docs (TASK.md / SUMMARY.md / REVIEW.md / DECISIONS.md). Ratified user best practice (goal `pio-r1-probe-sdk-session`, 2026-09-16); ratified further by direct user feedback against live code (goal `pio-r2-sandbox-environment`, 2026-09-18): comments state BEHAVIOUR only, proportionate to the code — no ruling citations/dates, no step/plan attributions, no deviation-register labels, no governance shorthand; keep load-bearing data comments (e.g., per-row provenance of a compiled table); give interim code ONE short forward-looking marker, not a justification narrative. Shipped comments are SELF-CONTAINED from the artifact's own perspective — no planning-doc markers (step numbers, D#/S# decision ids, spec § citations, ruling dates); a mechanical marker-scan of staged bytes is a standard close-gate verification row in `pio/` work
- **Ship only what is currently consumed; defer everything else.** An implementation without a current consumer is removed, not documented; features land together with their first real consumer (one home per concern). No dormant helpers, speculative seams, or forward-looking logic in committed code
- **Types may lead; implementations never outrun their consumer.** Contract shapes are cheap and declarative — introduce them early to pin direction. Behavioural code (parsers, adapters, algorithms) is written when a real consumer appears, informed by actual usage
- **Plain names over imported jargon.** Question vocabulary imported from spec/governing documents instead of mirroring it reflexively; name things for what they are (or become) — clear, precise common nouns
- **Seams live next to what needs them (seam proximity).** An injectable seam lives in the module whose behaviour depends on it — never a neutral utility home — and its why-it-exists rationale (what the suite drives through it deterministically; what production pays — typically nothing) sits ADJACENT to the declaration and its first consumer. Litmus: a reader finds the seam AND its purpose without leaving the file (owner note, 2026-09-21)
...
- **Cast & explicit-any discipline (`pio/` sources):** Production modules carry ZERO `as` casts and ZERO explicit `any` — audited mechanically per step close over added/changed bytes. Suites tolerate ONE documented cast seam each (e.g., `asEvent` for synthetic SDK event payloads) plus erased structural harness types; unproven test values cross via parse boundaries (`JSON.parse`) or `Object.assign` widening instead of assertions. When TypeScript law makes an assertion unavoidable, prefer a zero-runtime-effect type-level adaptation (e.g., a constructor alias WITHOUT the `abstract` modifier — TS forbids `new` on abstract-typed values and erasable syntax erases the difference) and record it in the step SUMMARY
...
**Biome** (`@biomejs/biome` 2.5.13 — **exact-pinned in both trees' manifests**, owner-directed cross-tree alignment during goal `capability-base`: the root manifest's range had resolved to a stale engine whose formatting of committed bytes conflicted with the gate's engine, and the pre-commit hook had silently re-flipped a staged suite into the gate-failing layout) is the project linter and formatter, configured in the single shared root `biome.json`:
