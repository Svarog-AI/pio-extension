...
- **CapabilityPackageConfig** — The config type for directory package capabilities. Defined in `capability-package.ts`. Fields: `name`, `skills`, `validation`, `readOnlyFiles`, `writeAllowlist`, `prepareSession`, `postValidate`, `postExecute`, `frontmatterSchemas` (optional). Default-exported from `config.ts`.
- **Direct Tools** — Non-AI capabilities consolidated in `src/direct-tools.ts`: init, delete-goal, list-goals, parent, create-issue, goal-from-issue. Have no prompts, no sub-sessions, no validation hooks. Exported as `setupDirectTools(pi)`.
