...
- **TASK.md Skills Section** — A `## Skills` section in every `TASK.md`, produced by the specification writer (`evolve-plan`) after analyzing step requirements against `<available_skills>`. Lists recommended skills with one-sentence justifications per entry. Treated by `execute-task` as a primary signal for skill loading — loaded before general heuristic scanning.
