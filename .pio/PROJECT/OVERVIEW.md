...
├── pio/                       # Standalone pio executable — self-contained npm package (goals pio-r1-probe-sdk-session R1 + pio-r2-sandbox-environment R2; purely additive, root files untouched)
...
│   ├── src/
│   │   ├── cli.ts             # Strict argument parser (pure) + main(argv, io?) dispatch: R1 grammar + `run` rewired to runCapability("probe"); lazy literal import("./sandbox/run.ts") is the only SDK path; net-shrunk (transient session-run verb removed); last-resort error boundary
...
│   │   ├── run-session.ts     # Dedicated top-session entry runSession(argv?, io?): strict two-value syntactic-only parse `<capability> --sessions-root <dir>`; capability gate before TTY preflight; ZERO static imports (dynamic thunks exactly ./probe.ts + ./sandbox/run.ts); own last-resort boundary
│   │   ├── session.ts         # Durable pi SDK session wiring: SessionManager.create(cwd[, sessionDir]) (engagement .sessions/top slot in-namespace) → stored runtime factory → createAgentSessionServices (defaults-only; agentDir honors PI_CODING_AGENT_DIR) → createAgentSessionFromServices → createAgentSessionRuntime (mirrors pi's own CLI seam verbatim)
