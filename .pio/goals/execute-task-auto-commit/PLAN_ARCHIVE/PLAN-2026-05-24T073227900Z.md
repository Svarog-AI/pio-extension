...
Add automatic `git commit` to pio workflow agents via a new reusable `pio-git` skill and prompt changes to both `execute-task` and `execute-plan` workflows. No capability code changes required.
...
### Step 1: Create pio-git skill

**Description**

Create `src/skills/pio-git/SKILL.md` — a reusable skill that defines how pio agents perform git operations. The skill documents:
...
- **No capability code changes:** Both `src/capabilities/execute-task.ts` and `src/capabilities/execute-plan.ts` remain unchanged. All behavior is prompt+skill-driven via the agent's access to the `bash` tool.
