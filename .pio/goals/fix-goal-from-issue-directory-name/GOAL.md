...
**The bug (confirmed live):** Calling `pio_goal_from_issue(name: "goal-from-issue-wrong-directory-name", issuePath: "goal-from-issue-wrong-directory-name.md")` should produce a session writing to `.pio/goals/goal-from-issue-wrong-directory-name/GOAL.md`. Instead, the create-goal session's allowed write target was `.pio/goals/fix-goal-from-issue-directory-name/GOAL.md` — a different directory entirely. The queue file (`task-goal-from-issue-wrong-directory-name.json`) correctly contains `goalName: "goal-from-issue-wrong-directory-name"`, but something between queue resolution and session creation is transforming the goal name. The exact point of failure requires investigation — it may involve how pi's framework resolves working directories from session params, or a mismatch in how `next-task.ts` passes params to `resolveCapabilityConfig`.
