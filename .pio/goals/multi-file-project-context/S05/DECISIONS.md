# Accumulated Decisions (through Step 5)
...
- **7 output file paths are finalized** from Step 2's `writeAllowlist`: `.pio/PROJECT/OVERVIEW.md`, `.pio/PROJECT/DEVELOPMENT.md`, `.pio/PROJECT/CONVENTIONS.md`, `.pio/PROJECT/GIT.md`, `.pio/PROJECT/ARCHITECTURE.md`, `.pio/PROJECT/DEPENDENCIES.md`, `.pio/PROJECT/GLOSSARY.md`. These are the canonical paths — all prompts, code, and documentation must reference these exact relative paths.
...
- **Prompt and skill files have no dedicated unit tests:** Markdown content under `src/prompts/*.md` and `src/skills/*/SKILL.md` is verified through programmatic checks (string matching, section presence) and manual review. This is consistent across all capability prompts and skill docs in the codebase.
