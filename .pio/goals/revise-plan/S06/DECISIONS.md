...
- **Shared skill directory name:** The planning skill lives at `src/skills/pio-planning/SKILL.md` instead of `src/skills/planning/SKILL.md`. All references must use `pio-planning`. This affected Steps 5 and 6.
...
- **Prompt slimming pattern:** Established in Step 5 (revise-plan.md) — short role definition → Setup → process steps that inline only capability-specific logic → Guidelines referencing `pio-planning` skill → Skill References section at end listing what to consult and from where. Step 6 applies the same pattern to create-plan.md.
