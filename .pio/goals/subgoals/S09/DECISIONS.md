...
## Path Resolution (from Dimension 8)

- **Centralized resolution strategy:** Fix is centralized in `resolveGoalDir` itself (optional params for nested resolution), so individual capability files need no changes if they go through `resolveGoalDir`.
- **17 function groups audited across 12 source files.** 6 require changes (new logic or new fields), remainder require no changes.
- **Explicit `params.workingDir` from spawning transition** is the immediate mechanism for subgoal path resolution.
