...
const createGoalTool = defineTool({
  ...
  async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
    ...
    if (fs.existsSync(workspaceDir) && fs.readdirSync(workspaceDir).length > 0) {
      return { content: [{ type: "text", text: `Workspace at "${params.workspacePrefix}" already exists. Call ask_user to let the human decide what to do (pick a new name, reuse existing, etc.).` }], details: {} };
    }
...
async function handleCreateGoal(args: string | undefined, ctx: ExtensionCommandContext) {
  ...
  if (fs.existsSync(workspaceDir) && fs.readdirSync(workspaceDir).length > 0) {
    ctx.ui.notify(`Workspace at "${workspacePrefix}" already exists. Call ask_user to let the human decide what to do.`, "warning");
    return;
  }
  fs.mkdirSync(workspaceDir, { recursive: true });
