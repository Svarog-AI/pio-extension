...
Use this to identify new capabilities, modules, or architectural changes that may warrant PROJECT file updates — even if they don't appear in \`DECISIONS.md\`.`,
  },
  {
    ...
    instructions: `Scan the goal workspace for step folders. Read \`SUMMARY.md\` from each one that exists. These provide ground truth of what was actually built:
...
**Subgoal-aware reading:** When scanning step folders, check for a \`subgoals/\` subdirectory inside each step folder. If present, this step spawned nested subgoals. For each subgoal workspace under \`subgoals/<name>/\`:
...
Treat the subgoal as a single unit — don't confuse subgoal step folders with parent step folders. The subgoal's \`COMPLETED\` marker signals that the parent step is complete.`,
  },
  {
    id: "read-decisions",
...
**DECISIONS.md may be missing, empty, or incomplete.** If it doesn't exist or has no relevant content, proceed using only the \`plan\` input and \`SUMMARY.md\` files. Note this in your final summary.`,
  },
  {
    id: "synthesize",
...
Cross-reference all three: if the \`plan\` input mentions a new capability module that a \`SUMMARY.md\` confirms was created, but \`DECISIONS.md\` doesn't mention it, still evaluate it for PROJECT file updates. Do not rely on \`DECISIONS.md\` alone.`,
  },
  {
    id: "filter-decisions",
...
- **Triggering sources:** Which \`DECISIONS.md\` entry, \`SUMMARY.md\` finding, or \`plan\` input item triggered each change
- **Sources available:** Note which sources were read (the \`plan\` input, \`DECISIONS.md\`, per-step \`SUMMARY.md\` files) and which were missing or empty
