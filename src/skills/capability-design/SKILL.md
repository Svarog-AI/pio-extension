...
- **`"loop"`** — a do-while block: `body: WorkflowPhase[]` (the repeating unit) + `repeatWhile(state)` evaluated **at the end of each full body pass** (do-while — never pre-checked, so ≥1 pass is guaranteed). `maxIterations` on the block counts full body passes. The container itself never receives an agent turn, so it **must not carry an `instructions` field** — it is never rendered and does nothing. Only the body phases carry `instructions` (the prompt compiler validates `instructions` only on standard phases and exempts `loop` containers).
