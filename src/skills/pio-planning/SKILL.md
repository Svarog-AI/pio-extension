...
6. **Priority Hierarchy for Plan Revision** — resolving conflicts between the requirements document, archived plans, and revision notes
...
Below the title, include a one-line summary of what the plan achieves, referencing the input requirements file for context.
...
2. **Read every file referenced in the input requirements file.** Trace dependencies, imports, and related code that will be affected by the change.
...
- **Stay within the input requirements file's scope.** Do not add steps for refactoring unrelated code, fixing style issues, or "while you're at it" improvements. Those belong in separate goals.
- **The input requirements file is read-only during planning.** Never modify it. Your output is `PLAN.md` only — no source code creation.
...
When a plan is rewritten after some steps are already completed, multiple sources of truth can conflict: the requirements document, the archived `PLAN.md`, and revision notes from the trigger step. Resolve conflicts using this priority hierarchy for implementation details:

**Revision notes > archived PLAN.md > requirements document**
...
- **Requirements document** — provides scope boundaries and high-level context. Use it to understand *what* should be built, but do not let its high-level description override specific *how* decisions already encoded in the archived plan.

### Scope vs. Implementation

The *requirements document* defines *what* should be built and the scope boundaries. The archived `PLAN.md` defines *how* it should be built. During revision, preserve the *how* — the concrete implementation decisions, step decomposition, and formatting choices already made by the planning agent.
...
- **Don't over-interview.** The user already documented their intent in the input requirements file — only ask when research genuinely revealed something unclear or when multiple valid paths exist. Keep to 2-3 exchange rounds total. If the path is clear, present findings briefly and move on.
- **If the input requirements file is too vague to plan against**, tell the user and suggest what needs clarification. Don't fill in blanks yourself — a vague goal produces a vague plan.
